...
public class ArmoredimskyboxProcedure {
	...
	private static void execute(@Nullable Event event, LevelAccessor world) {
		if ((world instanceof Level _lvl ? _lvl.dimension() : Level.OVERWORLD) == (ResourceKey.create(Registries.DIMENSION, new ResourceLocation("chaosket:armoredim")))) {
			{
				ResourceLocation _texturelocation = new ResourceLocation(("chaosket" + ":textures/" + "sky_box" + ".png"));
				RenderSystem.setShaderTexture(0, _texturelocation);
				Minecraft.getInstance().getTextureManager().bindForSetup(_texturelocation);
			}
			renderSkybox(2, 0, (float) (world.getTimeOfDay(Minecraft.getInstance().getPartialTick()) * 360.0F), 0, (int) (255 << 24 | 255 << 16 | 255 << 8 | 255), true);
		}
	}
}
